...
//  CovidAPIApp
//
//  Created by Satoshi Ota on 2021/08/03.
//
...
class ViewController: UIViewController {
    ...
    func setUpLabel(_ text: String, size: CGSize, centerX: CGFloat, y: CGFloat, font: UIFont, color: UIColor, _ parentVew:UIView) {
        let label = UILabel()
        label.text = text
        label.frame.size = size
        label.center.x = centerX
        ...
        label.font = font
        label.textColor = color
        parentVew.addSubview(label)
    }
    
    func setUpGradation() {
        let gradientLayer = CAGradientLayer()
        gradientLayer.frame = CGRect(x: 0, y: 0, width: view.frame.size.width, height: view.frame.size.height / 2)
        gradientLayer.colors = [colors.bluePurple.cgColor, colors.blue.cgColor, colors.yellow.cgColor]
